(() => {
  ...
  const launchCurtain = $("#launchCurtain");
  ...
  let wakeLock = null;
  const wakeSupported = "wakeLock" in navigator && !!navigator.wakeLock?.request;
  let wakeWarned = false;

  async function acquireWakeLock(forceToast = false) {
    if (!state.keepAwake || !wakeSupported || document.visibilityState !== "visible") return false;
    try {
      ...
      updateWakeUI();
      if (forceToast) toast("Screen will stay awake");
      return true;
    } catch (err) {
      updateWakeUI();
      if (forceToast && !wakeWarned) {
        wakeWarned = true;
        toast("Wake lock blocked on this device/app mode");
      }
      return false;
    }
  }

  async function releaseWakeLock(forceToast = false) {
    try { await wakeLock?.release(); } catch {}
    wakeLock = null;
    updateWakeUI();
    if (forceToast) toast("Screen wake lock off");
  }

  function updateWakeUI() {
    if (!wakeBtn || !wakeStateEl) return;
    const active = !!wakeLock;
    wakeBtn.setAttribute("aria-pressed", String(active));
    if (!state.keepAwake) wakeStateEl.textContent = "Off";
    else if (active) wakeStateEl.textContent = "On";
    else if (wakeSupported) wakeStateEl.textContent = "Auto";
    else wakeStateEl.textContent = "N/A";
  }
  ...
  function speak() {
    ...
    const preferred = voices.find(v => /Samantha|Aaron|Ava|Daniel/i.test(v.name)) || voices.find(v => /^en/i.test(v.lang));
...
const WAX = [
  ...
  "Low-temp dabs usually trade giant clouds for better flavor detail.",
  "High heat can feel stronger up front, but often burns off terps faster.",
  "Rosin can darken quickly at high temps—shorter heat and patience help.",
  "Fresh badder is often easier to portion than brittle shatter.",
  "Diamonds + sauce often dab best when mixed a little before loading.",
  "Saucy concentrates can splash if the draw is too aggressive.",
  "Dry concentrates may need a touch more heat soak than runny ones.",
  "Terpenes evaporate at lower temps than cannabinoids—timing changes the experience.",
  "A longer cool-down usually means smoother vapor and less throat bite.",
  "Smaller loads often taste better than overpacking the banger.",
  "Overheating once can affect flavor for the rest of the session.",
  "Color alone doesn’t tell the full story—texture and aroma matter too.",
  "Sticky concentrates are easier to handle with a slightly warm tool.",
  "Cold concentrates can be easier to portion cleanly.",
  "Whipping badder too much can change texture over time.",
  "Concentrates stored warm may lose aroma faster.",
  "A tiny dab at the right temp can outperform a big dab at the wrong temp.",
  "Different strains can like different temp ranges—some terps shine lower.",
  "Harsh hit? Try less heat or a longer cooldown before assuming the product is bad.",
  "If flavor disappears instantly, your banger may be too hot.",
  "Pooling oil after the hit can mean temp was too low or draw was too light.",
  "Dark residue after one dab usually points to excess heat.",
  "A smooth dab often comes from timing, not brute-force torching.",
  "Rosin usually rewards precision more than speed.",
  "Live products often smell louder because more volatile compounds remain.",
  "Shatter becomes easier to work with when slightly warmed—just don’t overdo it.",
  "Badder consistency can change with room temperature. That’s normal.",
  "Sugar textures can separate over time; a gentle mix can help consistency.",
  "Some concentrates vaporize in stages—flavor first, then heavier compounds.",
  "Short pulls can preserve flavor; monster rips can cool the banger too fast.",
  "The best temp is the one that gives flavor, comfort, and complete vaporization for your setup.",
  "Residual puddles aren’t always waste—sometimes they’re a sign to tweak airflow or timing.",
  "Terp-heavy dabs can feel stronger in aroma even with smaller doses.",
  "Consistent dab size makes it easier to learn your ideal timer.",
  "If every dab tastes the same, your temps may be too high.",
  "Flavor-first sessions usually start with patience, not extra flame."
];
...
const TOOL = [
  ...
  "A directional carb cap can move oil around and improve vaporization.",
  "Torch angle matters—heat the banger evenly to avoid hot spots.",
  "Overheating one side of the banger can cause uneven pooling.",
  "Quartz holds heat differently than thinner glass—your timer may need adjusting.",
  "A dirty cap can mute airflow and flavor more than people expect.",
  "Cotton swab cleanup right after a dab helps keep flavor consistent.",
  "Warm cleanup is easier than scraping burnt residue later.",
  "Too much airflow can cool the banger before the concentrate fully vaporizes.",
  "Too little airflow can make hits feel dense and harsh.",
  "Letting the banger cool between hits protects flavor and hardware.",
  "Repeated red-hot cycles can shorten banger life over time.",
  "A stable rig setup reduces spills, especially with small tables or soft surfaces.",
  "Keep your torch flame controlled—bigger flame isn’t always better heat.",
  "A clean water line improves pull feel and reduces splash.",
  "Too much water can create drag and make smooth pulls harder.",
  "Too little water can make hits feel hotter and harsher.",
  "Changing water regularly helps flavor stay fresh.",
  "A spinning cap works best when pearls and airflow are balanced.",
  "Terp pearls can help spread oil, but too many can choke airflow.",
  "If pearls aren’t spinning, check cap angle and inhale speed first.",
  "Quartz can look clean but still hold residue film—deep cleans help.",
  "Keep tools dry—water on a hot surface can cause splatter.",
  "Loading near the center of the banger usually gives more even melt.",
  "Touching the hottest part with the dab tool can leave sticky residue on the tool.",
  "Prepping your cap before the dab starts makes the session smoother.",
  "A dedicated station tray saves you from chasing tools mid-session.",
  "Silicone containers are handy, but glass jars often preserve aroma better long-term.",
  "Loose lids can dry concentrates out faster than expected.",
  "Store concentrates away from heat and direct sunlight.",
  "If your banger clouds up quickly, your cleaning routine probably needs a tune-up.",
  "A consistent torch routine beats random reheats every time.",
  "Reheating leftovers too hard can turn a decent finish into a harsh one.",
  "Your setup’s airflow path affects flavor more than people think.",
  "The best rig setup is the one you can repeat consistently without guesswork.",
  "Heat, cap, inhale—doing them in the same order helps build reliable results.",
  "A timer plus a repeatable torch pattern beats guessing by glow color.",
  "If the hit feels weak, adjust one variable at a time: heat, cooldown, or draw.",
  "Smooth sessions come from maintenance more than expensive gear.",
  "Clean quartz + proper cap + patient timing = the holy trinity."
];
...
clearAllBtn.addEventListener("click", () => {
  ...
  wakeBtn?.addEventListener("click", async () => {
    ...
    save();
    if (!state.keepAwake) await releaseWakeLock(true);
    else {
      const ok = await acquireWakeLock(true);
      if (!ok && state.running) {
        // keep setting enabled; will retry on visibility changes
      }
    }
    render();
  });
  ...
  document.addEventListener("visibilitychange", async () => {
    if (document.visibilityState === "visible") {
      if (state.running) {
        // keep timer accurate even if throttled in background
        state.remaining = computeRemaining();
        render();
      }
      if (state.keepAwake && state.running) acquireWakeLock();
    } else {
      // platform may auto-release; keep state/UI coherent
      wakeLock = null;
      updateWakeUI();
    }
  });
  ...
  function applyActivePresetOnLoad() {
    ...
    // After refresh/relaunch, restore safe non-running state instead of stale "running"
    state.running = false;
    state.startedAt = 0;

    // if timer was mid-run before close, keep remaining if valid; otherwise sync to preset
    if (!(state.remaining > 0 && state.remaining <= state.total)) state.remaining = state.total;
    state.elapsedBeforePause = 0;
  }
  ...
  // warm speech voices on iOS (can be flaky until loaded)
  if ("speechSynthesis" in window) {
    window.speechSynthesis.onvoiceschanged = () => { primeVoices(); };
    setTimeout(primeVoices, 200);
  }

  // smooth splash exit
  setTimeout(() => launchCurtain?.classList.add("hide"), 700);
  setTimeout(() => launchCurtain?.remove(), 1300);
